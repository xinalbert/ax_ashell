[简体中文](user-guide.zh.md) · [Documentation](README.md)

# AxShell User Guide

The former single-page guide has been split into focused pages so each workflow can evolve independently and receive its own screenshots.

- [Getting Started](getting-started.md)
- [Terminal And SSH](features/terminal-ssh.md)
- [Serial And Telnet](features/serial-telnet.md)
- [Workspace](features/workspace.md)
- [SFTP](features/sftp.md)
- [Appearance And Settings](features/appearance-settings.md)
- [Configuration Sync](features/configuration-sync.md)
- [Proxy And X11](features/proxy-x11.md)
- [Monitoring And Lifecycle](features/monitoring-lifecycle.md)
- [Local Data And Troubleshooting](features/local-data-troubleshooting.md)
- [Development And Packaging](development.md)
