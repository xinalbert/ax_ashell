...
| `src/app/actions/session.rs` | 会话连接、SSH 表单、local shell profile、tab 生命周期和工作区窗口迁移动作 | `move_workspace_group_to_new_window`，`return_workspace_to_main_window`，`restore_workspace_transfer`，`save_ssh_session_from_form`，`record_session_shortcut`，`connect_session_shortcut_if_matched`，`connect_ssh`，`open_saved_session_sftp_only`，`shutdown_all_backends`，`active_snapshot` | 改本地/SSH tab 创建、整个 `TabGroup` 的迁出/回迁、会话 `sftp_path` / `x11_forwarding` / `shortcut` 保存、SSH 表单加载/重置和高级项自动展开、saved SSH 只开 SFTP、动态快捷键连接、tab UI 状态回收、断线重试、关闭 tab/group、窗口退出或 active session 查询时 |
