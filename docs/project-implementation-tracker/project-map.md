...
| `src/terminal/element.rs` | terminal 行级 `ShapedLine` 缓存、字体 metrics、等宽保护与实时覆盖层绘制 | `TerminalElement`，`ensure_terminal_font_family_loaded`，`GridLayoutCache`，`GridLayoutKey`，`hovered_url_underlines`，`RowLayout` | 在 metrics / shape 前确保当前内置 terminal family 已注册；改终端 text/background/block 准备缓存、搜索/keyword 色彩失效键、Command/Ctrl 下的 URL/path 下划线、光标/选择/IME 或字体间距问题时 |
...
| `src/terminal/tab.rs` | terminal tab、共享行块 snapshot、滚屏复用、selection、高亮节流和 SSH 输入反馈计时 | `TabKind`，`TerminalTab`，`InputFeedbackTracker`，`new_serial_or_telnet`，`send_user_input`，`RenderSnapshot`，`RenderRow`，`SnapshotCache` | Serial/Telnet 复用终端渲染与 scrollback；SSH 键盘/IME 输入仅匿名记录到首个后续输出的反馈时间，连续输入合并、30 秒超时丢弃、重连重置；`feed` / resize / scroll 累积并 reset damage |
