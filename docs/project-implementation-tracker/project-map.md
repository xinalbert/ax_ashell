...
| `src/terminal/element.rs` | terminal 行级 `ShapedLine` 缓存、字体 metrics、等宽保护与实时覆盖层绘制 | `TerminalElement`，`TerminalComposition`，`paint_composition`，`GridLayoutCache`，`GridLayoutKey`，`hovered_url_underlines`，`RowLayout` | 在 metrics / shape 前确保当前内置 terminal family 已注册；改终端 text/background/block 准备缓存、搜索/keyword 色彩失效键、Command/Ctrl 下的 URL/path 下划线、光标/选择/IME/本地输入 overlay 或字体间距问题时 |
...
- 最近依据：`AGENTS.md`，`Cargo.toml`，`Cargo.lock`，`.github/workflows/ci.yml`，`src/session.rs`，`src/app/actions/terminal.rs`，`src/app/lifecycle/event_loop.rs`，`src/terminal/tab.rs`，`src/terminal/element.rs`，`docs/project-env-audit/current.md`
